/**
 * format.ts — Helpers de formato del portal (escape, fechas, moneda, badges,
 * iconos). Portado de `backend/src/ui/fragments.js` (sin la parte HTMX/OOB).
 */
...
export function fmtMoney(cents: unknown, currency = 'USD'): string {
  const n = Number(cents || 0) / 100
  const sym = ({ USD: '$', EUR: '€', COP: '$' } as Record<string, string>)[currency] || '$'
...
export function fmtPeriod(start: unknown, end: unknown): string {
  ...
  const fmt = (d: Date, withMonth: boolean) => d.toLocaleDateString('es-CO', withMonth
...
/** Clase + label de un badge según estado. */
export function badgeFor(status: unknown): { cls: string; label: string } {
  const [cls, label] = BADGES[String(status)] || ['', status ? String(status) : '—']
  return { cls, label }
}
...
export const ICONS: Record<string, string> = {
  ...
  store: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l1.5-5h15L21 9M3 9h18M3 9v10a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V9M9 13h6"/></svg>',
